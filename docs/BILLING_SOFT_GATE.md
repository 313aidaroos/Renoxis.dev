...
## Locked Wallet SKUs (hub)

| Key | Alias | Amount |
| --- | --- | --- |
| `renoxis.activate` | `renoxis-activate` | 5,000 Ixis one-time |
| `renoxis.agent.monthly` | `renoxis-monthly` | 5,000 Ixis/mo (revise off 30,000 — do **not** invent `renoxis.seat.monthly`) |
...
4. Local entitlement flags live in `localStorage` (`renoxis-billing-v1:<email>`) until Wallet redeem + entitlement callback.
5. Without SKUs live: CTAs still buy packs; redeem UI shows honest **SKU pending** (names the locked key). Soft-launch Confirm marks on-device flags only.
...
- Live catalog prices at 5,000 for both SKUs
